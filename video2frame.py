...
def extract_frames(video_path, frames_dir, overwrite=False, start=-1, end=-1, every=1):
    ...
    if every > 25 and len(frames_list) < 1000:  # this is faster for every > 25 frames and can fit in memory
        ...
        for index, frame in zip(frames_list, frames):  # lets loop through the frames until the end
            save_path = os.path.join(frames_dir, video_filename, "{:010d}.jpg".format(index+5000))  # create the save path
            if not os.path.exists(save_path) or overwrite:  # if it doesn't exist or we want to overwrite anyways
                cv2.imwrite(save_path, cv2.cvtColor(frame, cv2.COLOR_RGB2BGR), [int(cv2.IMWRITE_JPEG_QUALITY), 75])  # save the extracted image with low resolution
        ...
        for index in range(start, end):  # lets loop through the frames until the end
            ...
            if index % every == 0:  # if this is a frame we want to write out based on the 'every' argument
                save_path = os.path.join(frames_dir, video_filename, "{:010d}.jpg".format(index+5000))  # create the save path
                if not os.path.exists(save_path) or overwrite:  # if it doesn't exist or we want to overwrite anyways
                    cv2.imwrite(save_path, cv2.cvtColor(frame.asnumpy(), cv2.COLOR_RGB2BGR), [int(cv2.IMWRITE_JPEG_QUALITY), 75])  # save the extracted image with low resolution
                    saved_count += 1  # increment our counter by one
...
if __name__ == '__main__':
    # test it
    video_to_frames(video_path='clip2_Trim.mp4', frames_dir='dataset/classify_2/images', overwrite=True, every=3)